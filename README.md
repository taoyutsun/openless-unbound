...
- **歷史記錄**：可搜尋、複製與重用過去的語音輸入結果，並重新潤色或更換輸出風格。
...
`v1.3.18-1` 選擇性整合上游 OpenLess 1.3.18 中適合 Windows 與 Unbound 主線的穩定性與工作流程改善：

- 歷史記錄可用指定的內建或自訂風格重新潤色，不必重新錄音即可更換輸出格式。
- 文字無法插入目標 app 時，會顯示可選取全文並一鍵複製的復原卡片；Windows 版會維持無邊框、不搶焦點，關閉後不殘留透明視窗。
- Whisper-compatible WebSocket 連線加入握手 timeout，LLM 潤色 timeout 會依文字長度與串流狀態調整，降低網路異常時長時間卡住或長文過早逾時的情況。
- 偏好設定若有個別欄位格式損壞，會盡量保留仍可解析的 Unbound provider、快捷鍵與功能設定，不再直接回到整份預設值。
- 強化 Windows TSF 插入後恢復原輸入法的流程，並更新與輸入、建置安全性相關的依賴。

「允許非 TSF 兜底」開啟時，TSF 無法落字後會繼續嘗試模擬鍵盤或剪貼簿貼上；Windows 接受該操作後不會顯示復原卡片。關閉此選項時，TSF 明確失敗便會顯示復原卡片，方便手動複製完整文字。

本次仍不納入 Android APK runtime、手機遠端輸入、Less Computer / Cloud Agent 與大規模 mobile UI，維持 Windows 主線的範圍與穩定性。
...
4. 結果會自動插入游標位置；若插入流程明確失敗，畫面會顯示可選取與複製完整文字的復原卡片。
...
「允許非 TSF 兜底」預設用於提高特殊 app 的落字成功率：TSF 被拒絕時，程式會再嘗試模擬鍵盤或剪貼簿貼上。若要測試 TSF 本身或希望失敗時直接顯示復原卡片，可暫時關閉此選項。
