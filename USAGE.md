...
`v1.3.11-2` 起，繁體中文強制字形轉換改用台灣繁體字形表，避免「吃」被一般簡繁表轉成「喫」；一般語音輸入也不再因介面語言或繁中字形偏好而強制改成中文。未啟用即時翻譯時，英文語音與中英夾雜內容會維持原語言，只在內容本身包含中文時套用簡繁字形偏好。

### 落字失敗與非 TSF 兜底

「設定 → 通用 → 插入與剪貼簿」中的「允許非 TSF 兜底」控制 TSF 插入失敗後是否繼續嘗試其他方法：

- 開啟：改用模擬鍵盤輸入或剪貼簿貼上，提高特殊 app 的落字成功率。Windows 回報已送出後，通常不會顯示復原卡片。
- 關閉：不再嘗試上述方法；TSF 明確失敗時，顯示包含完整轉寫內容的復原卡片，可選取文字或按「複製完整文字」。

復原卡片不是輸入框偵測器，只在插入流程回報失敗時出現。Windows 桌面等沒有一般文字焦點的系統介面，不一定會產生同樣的失敗狀態；測試時可使用瀏覽器或一般 app 中沒有輸入框的區域。

日常使用建議開啟非 TSF 兜底；只有在檢查 TSF、避免模擬按鍵／剪貼簿操作，或測試復原卡片時暫時關閉。
...
6. 可手動選取回答中的局部文字，或按回答旁邊的複製按鈕複製整則回答。
7. 按 Esc 關閉浮窗。

目前劃詞追問不內建即時上網搜尋，也沒有外部 tool 調用流程。它適合針對已選取文字做解釋、整理、改寫與延伸提問；匯率、天氣、新聞等最新資訊不會被即時查詢。
...
`v1.3.18-1` 起，可在歷史記錄選擇既有項目，指定目前的內建或自訂風格重新潤色。這只重新處理已保存的轉寫文字，不需要重新播放或上傳錄音；原始內容仍會保留，方便比較不同風格的結果。
